...
func getRecommendedPage(page int) (*bytes.Buffer, *urørt.Page, error) {
	...
	// http get
	u := fmt.Sprintf("http://urort.p3.no/breeze/urort/TrackDTOViews?$filter=Recommended%%20ne%%20null&$orderby=Recommended%%20desc%%2CId%%20desc&$top=%d&$expand=Tags%%2CFiles&$inlinecount=allpages&playlistId=0&$skip=%d", pageLen, skip)

	log.Printf("getRecommendedPage: prøver å hente: %q", u)
